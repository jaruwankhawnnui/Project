...
const HardwareStock = () => {
  ...
  useEffect(() => {
    const fetchStockAndBorrowedItems = async () => {
      setLoading(true);
      try {
        const stockResponse = await fetch("http://172.25.176.1:1337/api/stockitems?populate=*");
        const borrowedResponse = await fetch("http://172.25.176.1:1337/api/borroweditems?populate=*");
  ...
  const handleBorrowItem = async () => {
    try {
      const response = await fetch("http://172.25.176.1:1337/api/borroweditems", {
        method: "POST",
        headers: {
  ...
  const handleReturnItem = async (id) => {
    try {
      const response = await fetch(`http://172.25.176.1:1337/api/borroweditems/${id}`, {
        method: "DELETE",
      });
