...
const StockPage = () => {
  ...
  useEffect(() => {
    const fetchStockItems = async () => {
      try {
        const response = await fetch("http://172.17.48.1:1337/api/equipment?populate=image");
        if (response.ok) {
            ...
            imageUrl:
              item.attributes.image?.data?.length > 0
                ? item.attributes.image.data[0].attributes.url.startsWith("http")
                  ...
                  : `http://172.17.48.1:1337${item.attributes.image.data[0].attributes.url}`
                : null,
          }));
  ...
  const handleUpdateStock = async (id, newAmount) => {
    try {
      const response = await fetch(`http://172.17.48.1:1337/api/equipment/${id}`, {
        method: "PUT",
        headers: {
